...
export default function FolderContributions() {
  ...
  function generateRanking() {
    ...
    console.log(typeRanking);
    if (typeRanking) {
      console.log("PORCENTAJE");
      newRanking = newRanking.map((value) => ({
        ...value,
        points: ((value.points / total) * 100).toFixed(2) + "%", // Convertir a porcentaje con 2 decimales
      }));
    }
  ...
  async function getContributions() {
    let url = `/api/v1/contributions/${owner}/${repo}/between_time?login=${username}&path=${path}`;
    let minYear, maxYear;
    if (startDate !== null) {
